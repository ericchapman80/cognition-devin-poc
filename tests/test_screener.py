...
from trading_agents.screener import (
    ScreenerPick,
    ScreenerResult,
    TradingScreener,
...
class TestTradingScreener:
    @patch("trading_agents.screener.TechnicalAnalyzer")
    @patch("trading_agents.screener.MarketDataProvider")
    def test_gather_ticker_data_success(self, mock_data_cls, mock_ta_cls):
        ...
        llm = MagicMock()
        screener = TradingScreener(config=config, llm=llm)
