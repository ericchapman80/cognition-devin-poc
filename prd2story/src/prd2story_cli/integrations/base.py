"""Base classes for prd2story AI assistant integrations.
...
class IntegrationBase(ABC):
    ...
    CONTEXT_MARKER_START = "<!-- PRD2STORY START -->"
    CONTEXT_MARKER_END = "<!-- PRD2STORY END -->"
    ...
    @abstractmethod
    def setup(self, project_root: Path, org_name: str = "") -> list[Path]:
...
class MarkdownIntegration(IntegrationBase):
    ...
    def setup(self, project_root: Path, org_name: str = "") -> list[Path]:
        ...
        ctx_path = self._install_context_file(project_root, org_name)
        if ctx_path:
            created.append(ctx_path)

        return created

    def _install_context_file(self, project_root: Path, org_name: str = "") -> Path | None:
